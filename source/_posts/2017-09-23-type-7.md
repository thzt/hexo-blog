...
即`let`表达式可以看做下式的缩写，<span data-katex="(\lambda x:T_1.t_2)t_1"></span>。
...
对`t_1`求值的时候有一个限制，那就是<span data-katex="t_1"></span>中不能出现<span data-katex="x"></span>，
...
引入了<span data-katex="fix_\sigma"></span>之后，`letrec`表达式就可以看做`let`表示的语法糖了，
