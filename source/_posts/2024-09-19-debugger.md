...
但如果仅仅只有这些，对于了解语言技术而言，是不够的。

此外，我看到 市面上有太多资料介绍 parser 了，

但其实 parser 只是故事的开始罢了。
...
最常见的文法，就是 [上下文无关文法](https://baike.baidu.com/item/%E4%B8%8A%E4%B8%8B%E6%96%87%E6%97%A0%E5%85%B3%E6%96%87%E6%B3%95/2001908)。
...
其中，`VariableDeclaration = 'let' Identifier '=' Value` 这一行成为一个产生式，

左边 `VariableDeclaration` 是非终结符（它会被展开成右边），

右边 `'let'` `'='` 成为终结符（它不能再被展开了），
...
所以需要文法尽量准确（无歧义），对于同一段字符串，有且只有一种转换方式。
...
有很多文章，花了大量篇幅介绍 parser 的编写过程，

其实日常工作中，从零开始写一个 parser 的时候很少。

即便是有，那么手写**递归下降语法解析器**，就已经能满足大多数场景了。
...
解释器的实现套路在 第二篇 [如何编写一个简单的解释器](https://thzt.github.io/2021/01/15/eval/) 中有过介绍，每一个 AST 节点，对应一个 evalXXX 函数，然后递归调用。
...
然后在特定阶段（需要停下来的时候），通过 `yield` 把状态信息返回出来。
...
我们知道软件领域，当今正在经历翻天覆地的变革，

但不论怎样变革，也只是一种潮流/趋势，而知识本身并没有变化太多。

<br/>

以上介绍的内容，颇为小众。

然而这些小众知识，反而能让我们在实际工作中如虎添翼、立竿见影。
